...
TypedStorage.EncryptedObjectStore = function (storage, objType, typedStorage) {
  var self = new TypedStorage.ObjectStore(storage, objType, typedStorage);
  
  //retrieve the object or null stored with a specified key
  self.get = function(key, cb) {
    cb(self.decryptObject(self.getObject(self._storage, self.makeKey(key))));
  };
  ...
  self.put = function(key, value, cb) {
    var canceled = ! self._typedStorage.dispatchEvent('change',
      {target: key, storageType: self, eventType: 'change', value: value});
    self.setObject(self._storage, self.makeKey(key), self.encryptObject(value));
    self._typedStorage.setLastModified();
    cb(true);
  };
  
  self.encryptObject = function(value) {
    return Crypto.AES.encrypt(value, self.keyPhrase);
  };
  
  self.decryptObject = function(Value) {
    return Crypto.AES.decrypt(value, self.keyPhrase);
  }
  
  self.setKeyPhrase = function(keyPhrase) {
    // XX should expire this internally
